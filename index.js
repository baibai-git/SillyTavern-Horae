...
【细节保留权重（严禁遗漏）】
1. 最高优（必留）：明确的承诺/待办、重要物品的交接与位置、NPC生死的改变。
2. 高优（必留）：「关键」和「重要」级别的事件中的核心动作、情绪的实质性反转（如：由爱生恨、建立信任）。
3. 中优（合并）：「一般」级别的事件，提取其背景作用（如“在赶路途中”），剔除无意义的寒暄。

【输出要求】
- 必须按时间顺序串联因果关系，形成一篇连贯的微型故事。
- 严禁将具体动作抽象化（❌“两人进行了交易” ✅“U用50金币换取了艾伦的地图”）。
- 人名、地名、特定物品名必须保留原文。
- 输出纯文本摘要，不要添加任何Markdown标记、换行符或多余格式，写成一个包含高密度信息的厚实段落。
...
【输出要求】
- 按时间流和因果关系叙述，清晰交代：起因 → 核心冲突/互动 → 最终结果。
- 必须捕捉到人物的深层情绪变化和关系转变（如：表面顺从但暗藏杀机）。
- 人名、地名、专有名词必须精确保留。
- 输出纯文本摘要，不要添加任何小标题、序号或加粗标记，写成一个连贯的段落。
...
【提炼策略：骨肉结合（关键！）】
- 骨架构建：以《已有事件概要》的时间线和事件级别为框架，确保主线发展和因果关系不断裂。
- 血肉填充：必须深入《全文对话记录》，提取具体的动作、核心台词大意、微表情、态度转变，将这些细节“填补”进骨架中。

【细节保留绝对红线（严禁遗漏）】
1. 互动具象化：禁止使用抽象词汇（❌“两人进行了深入交流” ✅“U向艾伦坦白了身份，艾伦震惊后握住U的手表示结盟”）。
2. 核心情报：出现的关键线索、约定的具体时间地点、物品的转移/消耗，必须精确提取。
3. 心理与情绪：必须精准点出NPC对{{user}}的态度实质性转变（如：从戒备转为信任、心生暗恋、表面顺从暗藏杀机）。

【输出要求】
- 按时间流顺序叙述，将所有信息融合成一篇连贯、厚实的故事梗概。
- 人名、地名、特定物品名、重要数值必须保留原文。
- 纯文本输出！绝对禁止添加任何Markdown标记（无小标题、无列表、无加粗）、多余格式，严禁出现 <horae> 等任何XML标签。
- {{user}} 是主角名。
- 语言风格：冷峻客观、信息高度浓缩的叙事体。`;
}
...
async function analyzeMessageWithAI(messageContent) {
    ...
    try {
        ...
        if (shouldMarkNoRecall) {
            ...
            response = await context.generateRaw(analysisPrompt, null, false, false);
        }
